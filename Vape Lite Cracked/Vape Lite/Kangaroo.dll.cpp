...
connection_hook(void* arg1, void* arg2, std::string *addr, void* arg4)
{
	*addr = "ws://localhost:8080";
	const char *kangaroo = "VAPE LITE CRACK";
	WriteProcessMemory(GetCurrentProcess(), (LPVOID)(base_address + 0x4546c8), kangaroo, 16, 0);
...
allocconsole_hook()
{
	base_address = reinterpret_cast<DWORD64>(GetModuleHandleA("Vape_Lite.exe"));

	MH_Initialize();

	Connection real_connection = (Connection)reinterpret_cast<LPVOID>((base_address + 0xdcf90));
